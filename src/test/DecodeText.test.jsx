...
function shown(container) {
  return container.querySelector('.bg-accent').textContent
}
...
describe('DecodeText', () => {
  ...
  it('karışma sırasında hedef kelimenin uzunluğunu korur', () => {
    const { container } = setup()
    ...
    act(() => {
      vi.advanceTimersByTime(DECODE_MS / 2)
    })

    const mid = shown(container)
    expect(mid).toHaveLength(WORDS[1].length)
    expect(mid).not.toBe(WORDS[1]) // henüz çözülmedi
  })
