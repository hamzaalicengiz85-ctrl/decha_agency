...
export default function InboxPanel({ table }) {
  ...
  return (
    <div>
      <h2 className="font-display text-title font-bold uppercase text-accent">
      ...
      {rows.length === 0 && !error ? (
        <p className="panel p-5 font-mono text-meta text-fg-muted">Henüz kayıt yok.</p>
      ) : null}
        ...
        {rows.map((row) => (
          <li key={row.id} className="panel p-4">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <p className="font-display text-body font-bold uppercase text-accent">{row.name}</p>
              <p className="num font-mono text-label text-fg-subtle">
                {formatDate(row.created_at)}
              </p>
            </div>
            <p className="mt-1 font-mono text-label text-fg-muted">{row.email}</p>

            {isMessages ? (
              <p className="mt-3 whitespace-pre-wrap text-caption leading-relaxed text-fg-muted">
                {row.message}
              </p>
            ) : (
              <p className="num mt-3 font-mono text-meta text-fg-muted">
