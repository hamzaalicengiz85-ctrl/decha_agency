...
export default function Process() {
  ...
  return (
    <Section
      ...
      <div className="mt-12">
        <ol className="panel brackets stagger grid gap-px bg-line/35 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((item, index) => (
            <li key={item.step} className="bg-bg p-6">
                <div className="flex items-baseline gap-2">
                  <span className="num font-display text-stat font-bold text-accent">
                    {item.step}
                  </span>
                  <span className="hatch h-2 flex-1" aria-hidden="true" />
                </div>
                <h3
                  className="mt-4 font-display text-body font-bold uppercase text-fg"
